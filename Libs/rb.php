	...
	class OODBBean implements \IteratorAggregate, \ArrayAccess, \Countable, Jsonable {
		...
		private function getOwnList($type, $redbean) {
			...
			if ($this->getID()) {
				...
				$firstKey = count($this->withParams) > 0 ? key($this->withParams) : 0;
				if (is_int($firstKey)) {
					...
					if (!$selected || isset($selected[$key])) {
						if (is_array($value)) {
							if (isset($value['_type'])) {
								$bean = $this->beanHelper->getToolbox()
														 ->getRedBean()
		...
		public function export($meta = FALSE, $parents = FALSE, $onlyMe = FALSE, $filters = array()) {
			...
			foreach ($this as $key => $value) {
				if (!$onlyMe && is_array($value)) {
					...
					if ($hasFilters) { //has to be on one line, otherwise code coverage miscounts as miss
		...
		public function one($type) {
			return $this->beanHelper->getToolBox()
									->getRedBean()
									->load($type, $this->id);
		}
		...
		public function countOwn($type) {
			...
			if ($this->getID()) {
				...
				if (!self::$useFluidCount) {
					$count = $this->beanHelper->getToolbox()
											  ->getWriter()
											  ->queryRecordCount($type, array(), $sql, $bindings);
				} else {
					$count = $this->beanHelper->getToolbox()
											  ->getRedBean()
											  ->count($type, $sql, $bindings);
				}
			}
			$this->clearModifiers();
			return (int)$count;
		...
		public function countShared($type) {
			...
			$count = 0;
			if ($this->getID()) {
				$count = $redbean->getAssociationManager()
								 ->relatedCount($this, $type, $this->withSql, $this->withParams);
			}
			$this->clearModifiers();
		...
		public function &aggr($list, $property, $type = NULL) {
			...
			$beans = $this->beanHelper->getToolBox()
									  ->getRedBean()
									  ->batch($type, $ids);
...
namespace RedBeanPHP\QueryWriter {
	...
	abstract class AQueryWriter {
			...
			if ($joinType == 'shared') {
				...
				if (isset($globalAliases[$type])) {
					...
					if ($aliases && count($aliases) === 1) {
						...
						$assocTable = $this->getAssocTable(array(
		...
		public function queryRecordLink($sourceType, $destType, $sourceID, $destID) {
			...
			$sqlFilterStr = '';
			if (count(self::$sqlFilters)) {
				$linkType     = $this->getAssocTable(array(
					$sourceType,
					$destType
				));
		...
		public function queryRecursiveCommonTableExpression($type, $id, $up = TRUE, $addSql = NULL, $bindings = array(), $selectForm = FALSE) {
			...
			SELECT {$selectForm} FROM redbeantree {$sql};", $bindings);
			return $rows;
		}
...
namespace RedBeanPHP\QueryWriter {
	...
	class MySQL extends AQueryWriter implements QueryWriter {
		...
		public function __construct(Adapter $adapter, $options = array()) {
			...
			$this->encoding = $this->adapter->getDatabase()
											->getMysqlEncoding();
			$me             = $this;
		...
		public function createTable($type) {
			...
			$charset_collate = $this->adapter->getDatabase()
...
namespace RedBeanPHP\QueryWriter {
	...
	class CUBRID extends AQueryWriter implements QueryWriter {
		...
		public function createTable($table) {
			$sql = 'CREATE TABLE ' . $this->esc($table) . ' ("id" integer AUTO_INCREMENT, CONSTRAINT "pk_' . $this->esc($table, TRUE) . '_id" PRIMARY KEY("id"))';
		...
		protected function processSharedAdditions($bean, $sharedAdditions) {
			foreach ($sharedAdditions as $addition) {
				if ($addition instanceof OODBBean) {
					$this->oodb->getAssociationManager()
	...
	class AssociationManager extends Observable {
		...
		protected function associateBeans(OODBBean $bean1, OODBBean $bean2, OODBBean $bean) {
			...
			try {
				...
				if (!$this->writer->sqlStateIn($exception->getSQLState(), array(QueryWriter::C_SQLSTATE_INTEGRITY_CONSTRAINT_VIOLATION), $exception->getDriverDetails())) {
					throw $exception;
				}
			}
...
namespace RedBeanPHP {
	...
	class TagManager {
		...
		public function tagged($beanType, $tagList, $sql = '', $bindings = array()) {
			$tags    = $this->extractTagsIfNeeded($tagList);
			$records = $this->toolbox->getWriter()
									 ->queryTagged($beanType, $tags, FALSE, $sql, $bindings);
		...
		public function taggedAll($beanType, $tagList, $sql = '', $bindings = array()) {
			$tags    = $this->extractTagsIfNeeded($tagList);
			$records = $this->toolbox->getWriter()
									 ->queryTagged($beanType, $tags, TRUE, $sql, $bindings);
		...
		public function countTaggedAll($beanType, $tagList, $sql = '', $bindings = array()) {
			...
			return $this->toolbox->getWriter()
								 ->queryCountTagged($beanType, $tags, TRUE, $sql, $bindings);
		}
		...
		public function countTagged($beanType, $tagList, $sql = '', $bindings = array()) {
			$tags = $this->extractTagsIfNeeded($tagList);
			return $this->toolbox->getWriter()
								 ->queryCountTagged($beanType, $tags, FALSE, $sql, $bindings);
		}
	}
}
	...
	class LabelMaker {
		...
		public function dispenseLabels($type, $labels) {
			...
			foreach ($labels as $label) {
				$labelBean       = $this->toolbox->getRedBean()
												 ->dispense($type);
...
namespace RedBeanPHP {
	...
	class Facade {
		...
		public static function useISNULLConditions($mode) {
			self::getWriter()
				->flushCache(); /* otherwise same queries might fail (see Unit test XNull) */
		...
		public static function addDatabase($key, $dsn, $user = NULL, $pass = NULL, $frozen = FALSE, $partialBeans = FALSE, $options = array(), $beanHelper = NULL) {
			...
			if (!is_null($beanHelper)) {
				self::$toolboxes[$key]->getRedBean()
									  ->setBeanHelper($beanHelper);
			}
		}
		...
		public static function getQueryCount() {
			return self::$adapter->getDatabase()
								 ->getQueryCount();
		}
		...
		public static function usePartialBeans($yesNoBeans) {
			return self::$redbean->getCurrentRepository()
								 ->usePartialBeans($yesNoBeans);
		}
		 ...
		 * @return string
		 */
		public static function look($sql, $bindings = array(), $keys = array(
			'selected',
			'id',
			'name'
		),                          $template = '<option %s value="%s">%s</option>', $filter = 'trim', $glue = '') {
			return self::getLook()
					   ->look($sql, $bindings, $keys, $template, $filter, $glue);
		}
		 ...
		 * @return array
		 */
		public static function diff($bean, $other, $filters = array(
			'created',
			'modified'
		),                          $pattern = '%s.%s.%s') {
	...
	class DuplicationManager {
		...
		public function dup(OODBBean $bean, $trail = array(), $preserveIDs = FALSE) {
			if (!count($this->tables)) {
				$this->tables = $this->toolbox->getWriter()
											  ->getTables();
			}

			if (!count($this->columns)) {
				foreach ($this->tables as $table) {
					$this->columns[$table] = $this->toolbox->getWriter()
														   ->getColumns($table);
				}
			}
	...
	class ArrayTool {
		...
		public static function flat($array, $result = array()) {
			foreach ($array as $value) {
				if (is_array($value))
					$result = self::flat($value, $result); else $result[] = $value;
			}
			return $result;
		}
	}
		...
		public function csv($sql = '', $bindings = array(), $columns = NULL, $path = '/tmp/redexport_%s.csv', $output = TRUE, $options = array(
			...
			if ($columns)
				if (PHP_VERSION_ID >= 505040)
					fputcsv($handle, $columns, $delimiter, $enclosure, $escapeChar); else fputcsv($handle, $columns, $delimiter, $enclosure);
			$cursor = $this->toolbox->getDatabaseAdapter()
									->getCursor($sql, $bindings);
			while ($row = $cursor->getNextItem()) {
				if (PHP_VERSION_ID >= 505040)
					fputcsv($handle, $row, $delimiter, $enclosure, $escapeChar); else fputcsv($handle, $row, $delimiter, $enclosure);
			}
			fclose($handle);
			if ($output) {
				$file = basename($path);
...
namespace RedBeanPHP\Util {
	...
	class Look {
		 ...
		 * @return string
		 */
		public function look($sql, $bindings = array(), $keys = array(
			'selected',
			'id',
			'name'
		),                   $template = '<option %s value="%s">%s</option>', $filter = 'trim', $glue = '') {
...
namespace RedBeanPHP\Util {
	...
	class Diff {
		 ...
		 * @return array
		 */
		public function diff($beans, $others, $filters = array(
			'created',
			'modified'
		),                   $format = '%s.%s.%s', $type = NULL) {
			$diff = array();
